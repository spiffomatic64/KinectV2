...
#define AVAILABILITYCHANGE_WAIT_OBJECT WAIT_OBJECT_0 + 5
...
#define AUDIO_WAIT_OBJECT WAIT_OBJECT_0 + 9
...
uint32 FKinectSensor::Run(){
	...
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ KinectSensor thread run without a right initialization!"));
		return 0;
	}
	...
	while (!bStop) {
		...
		HANDLE handles[] = {	//Do not change the order of this array! if you do the switch below will redirect events to the wrong handler!
			...
			reinterpret_cast<HANDLE>(AvailableChangeEventHandle),
		...
		case AVAILABILITYCHANGE_WAIT_OBJECT:
		{
			sensorState = FALSE;

			TComPtr<IIsAvailableChangedEventArgs> pEventData = nullptr;
			hr = m_pKinectSensor->GetIsAvailableChangedEventData(AvailableChangeEventHandle, &pEventData);
			if (SUCCEEDED(hr))
			{
				pEventData->get_IsAvailable(&sensorState);
				pEventData.Reset();
				isAvailabilityChanged = true;
				UE_LOG(LogTemp, Log, TEXT("------------------ KinectSensor GetIsAvailableChangedEventData: %d"), sensorState);
			}
			else
			{
				UE_LOG(LogTemp, Error, TEXT("------------------ KinectSensor GetIsAvailableChangedEventData FAILED!"));
			}
		}
		break;
...
bool FKinectSensor::Init()
{
	...
	if (FAILED(hr))
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect GetDefaultKinectSensor FAILED!"));
		return false;
	}

	if (!m_pKinectSensor || FAILED(hr))
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect Initializing FAILED!"));
		return false;
	}

	hr = m_pKinectSensor->SubscribeIsAvailableChanged(&AvailableChangeEventHandle);
	if (FAILED(hr))
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect SubscribeIsAvailableChanged FAILED!"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect CoordinateMapper FAILED!"));
		return false;
	}
	...
	if (FAILED(hr))
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect open FAILED!"));
		return false;
	}
...
void FKinectSensor::StartSensor()
{
	if (!pKinectThread)
	{
		...
		if (!this->IsRunning())
		{
			UE_LOG(LogTemp, Error, TEXT("------------------ Kinect thread creation failed!"));
		}
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect thread not started"));
	}
}
...
void FKinectSensor::Exit()
{
	...
	m_pKinectSensor->UnsubscribeIsAvailableChanged(AvailableChangeEventHandle);
