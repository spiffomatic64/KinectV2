...
{
	if (TheJointType != EJoint::JointType_OFF) {
...
UKinectEventManager* UKinectFunctionLibrary::GetKinectEventManager()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect GetKinectEventManager not bound!"));
	}
...
void UKinectFunctionLibrary::StartSensor()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("------------------ Kinect StartSensorEvent not bound!"));
	}
}
...
bool UKinectFunctionLibrary::isKinectAvailable()
{
	TComPtr<IKinectSensor> pKinectSensor;
	HRESULT hr;
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = pKinectSensor->get_IsAvailable(&isAvailable);
		if (SUCCEEDED(hr) && isAvailable == 1)
		{
			return true;
		}
	}
