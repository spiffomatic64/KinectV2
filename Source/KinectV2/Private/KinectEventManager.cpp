...
void UKinectEventManager::KinectAvailabilityChanged(bool available)
{
	if (OnKinectAvailabilityChanged.IsBound())
	{
		OnKinectAvailabilityChanged.Broadcast(available);
...
{
	if (Player.GetValue() == EAutoReceiveInput::Disabled)
	{
		UE_LOG(LogTemp, Warning, TEXT("Player receive input disabled!"));
	}
