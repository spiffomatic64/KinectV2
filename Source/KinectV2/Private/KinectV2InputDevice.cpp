...
{
	UE_LOG(LogTemp, Log, TEXT("------------------ CreateInputDevice for Kinect"));
				...
				KinectManager(nullptr)
{
	...
	UE_LOG(LogTemp, Log, TEXT("------------------ Kinect bindRaw events"));
	...
	if (temp)
		KinectManager = NewObject<UKinectEventManager>(temp);

	if (KinectManager){
		KinectManager->AddToRoot();
	}
...
void FKinectV2InputDevice::SendControllerEvents(){
	...
	if (KinectSensor.IsValid() && KinectSensor->IsRunning())
	{
		...
		if (KinectManager){
			...
			if (newBodyFrame){
				KinectManager->ProcessNewBodyFrameEvent(Frame, MessageHandler);
				//if (GEngine)
				//{
				//	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("SEND NEW BODY FRAME!"));
				//}
				if (KinectManager->GetControlMouse())
				{
					MessageHandler->OnMouseMove();
				}
			}
			if (KinectManager->OnNewKinectColorFrame.IsBound()){
				KinectSensor->UpdateColorTexture(ColorFrame);
				KinectManager->OnNewKinectColorFrame.Broadcast(ColorFrame);
				//if (GEngine)
				//{
			...
			if (KinectManager->OnNewKinectDepthFrame.IsBound())
			{
				KinectSensor->UpdateDepthFrameTexture(DepthFrame);
				KinectManager->OnNewKinectDepthFrame.Broadcast(DepthFrame);

			}

			if (KinectManager->OnNewKinectInfraredFrame.IsBound())
			{
				KinectSensor->UpdateInfraredTexture(InfraredFrame);
				KinectManager->OnNewKinectInfraredFrame.Broadcast(InfraredFrame);
			}

			if (KinectManager->OnBodyIndexFrameEvent.IsBound())
			{
				KinectSensor->UpdateBodyIndexTexture(BodyIndexFrame);
				KinectManager->OnBodyIndexFrameEvent.Broadcast(BodyIndexFrame);
			}

			if (KinectSensor->IsAvailabilityChanged())
			{
				KinectManager->KinectAvailabilityChanged(KinectSensor->KinectIsAvailable());
			}
		}
	}
}
